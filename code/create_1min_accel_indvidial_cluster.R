...
## split the data into 200 partitions
K      <- 200
inx_sp <- split(1:nid, rep(1:K, each=ceiling(nid/K))[1:nid])
## get indices associated with this particular array
inx_k <- inx_sp[[arr_inx]]
...
## save the current array data
save(list=c(accel_mat_nms[inx_k],impute_mat_nms[inx_k]), file=file.path(data_path_processed,paste0("1min_accel_",arr_inx,".Rdata")))
