...
We have found that functional principal components analysis (fPCA) provides a relatively fast means of imputing missing data that performs well in practice. Essentially this procedure assumes there is a smooth underlying acceleration function (on the log scale) which we measure with error. Here we use an ultra fast algorithm @fpca_face which is implemented in the **R** package **refund** @refundR. One drawback of fPCA in this context is that the result is not guaranteed to be non-negative, so we truncate 
